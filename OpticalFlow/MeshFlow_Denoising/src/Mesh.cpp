...
bool Quad::getBilinearCoordinates(const cv::Point2f& pt, vector<double>& coefficient) const
{
    ...
    else
    {
        ...
        // V10.x, V10.y, V11.x, V11.y, pt.x, pt.y, k1, k2); 		printf("(%lf, %lf, %lf, %lf) -> (%lf, %lf, %lf, %lf)\n", a_x, b_x,
        // c_x, d_x, a_y, b_y, c_y, d_x); 		printf("(%lf, %lf, %lf, %lf) -> (%lf, %lf, %lf)\n", tmp1, tmp2, tmp3, tmp4, bigA, bigB,
        // bigC);
        return false;
    }
                ...
                if (flag)
                {
                    ...
                    if (q1.isPointIn(ptInSrc))
                    {
                        for (int c = 0; c < 3; c++)
                        {
                            ...
                            dst.at<cv::Vec3b>(
                                i + gap, j + gap)[c] = (tmpCoe[0] * f0 * src.at<cv::Vec3b>(int(tmpV00.y), int(tmpV00.x))[c]
