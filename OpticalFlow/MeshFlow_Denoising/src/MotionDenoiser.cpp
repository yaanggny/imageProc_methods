...
#include "opencv4_defines.h"

MotionDenoiser::MotionDenoiser(const char* name)
{
    m_frames   = GetFrames(name, m_fps);
	m_frames.erase(m_frames.begin(), m_frames.begin()+ m_frames.size()-50);
...
void MotionDenoiser::MotionEstimation()
{
    ...
    for (int i = 1; i < m_frameNum; i++)
    {
        ...
        printf("%03d/%3d\b\b\b\b\b\b\b", i, m_frameNum);
    }
    printf("[DONE]\n");
}
...
void MotionDenoiser::SaveResult(const char* name)
{

	cv::VideoWriter writer;
	writer.open(name, CV_FOURCC('X', 'V', 'I', 'D'), m_fps, m_size);
	if (!writer.isOpened()) 
	{
	...
	{
		writer << dst[i];
		printf("Write:  %03d/%3d\b\b\b\b\b\b\b", i, m_frameNum);
	}
}
