...
void CPM::SetStereoFlag(int needStereo) { _isStereo = needStereo; }

void CPM::SetStep(int step) { _step = step; }
...
int CPM::Matching(FImage& img1, FImage& img2, FImage& outMatches)
{
    ...
    int numV = gridw * gridh; // block center rather than the cross points of the grid
    int numV2 = numV;
    ...
    for (int i = 0; i < nLevels; i++)
    {
        _pydSeedsFlow[i].allocate(2, numV); // each block has a flow (same size as the seeds)
        _pydSeedsFlow2[i].allocate(2, numV2);
    }

    _seeds.allocate(2, numV);      // seeds coordinates
    _neighbors.allocate(12, numV); // save neighbor index
    ...
    _kLabels.allocate(w, h); // each block is labeled by the blockIndex
    for (int i = 0; i < numV; i++)
    {
        int x = _seeds[2 * i];
        int y = _seeds[2 * i + 1];
    ...
    // FImage goodMatchPairs;
    int validMatchCnt = CheckFlowValidation(_seeds, _pydSeedsFlow[0], _pydSeedsFlow2[0], _kLabels2, validFlag);
    seedsFlow.copyData(_pydSeedsFlow[0]);

    if (!outMatches.matchDimension(4, validMatchCnt, 1))
    {
...
void CPM::imDaisy(FImage& img, UCImage& outFtImg)
{
    ...
    cv::Ptr<cv::xfeatures2d::DAISY> daisy = cv::xfeatures2d::DAISY::create(5, 3, 4, 8, cv::xfeatures2d::DAISY::NRM_FULL,
                                                                           cv::noArray(), false, false);
...
int CPM::CheckFlowValidation(IntImage& seeds, FImage& seedsFlow, FImage& seedsFlow2, IntImage& kLabel2, int* valid)
{
    int w = kLabel2.width();
    ...
    for (int i = 0; i < numV; i++)
    {
        ...
        int x2 = x + u;
        int y2 = y + v;
        if (x < b || x >= w - b || y < b || y >= h - b || x2 < b || x2 >= w - b || y2 < b || y2 >= h - b
            || sqrt(u * u + v * v) > _maxDisplacement || abs(u) >= UNKNOWN_FLOW || abs(v) >= UNKNOWN_FLOW)
        {
            valid[i] = 0;
            numValid--;
            continue;
...
int CPM::Propogate(FImagePyramid& pyd1, FImagePyramid& pyd2, UCImage* pyd1f, UCImage* pyd2f, int level, float* radius,
                   int iterCnt, IntImage* pydSeeds, IntImage& neighbors, FImage* pydSeedsFlow, float* bestCosts)
{
...
void CPM::PyramidRandomSearch(FImagePyramid& pyd1, FImagePyramid& pyd2, UCImage* im1f, UCImage* im2f, IntImage* pydSeeds,
                              IntImage& neighbors, FImage* pydSeedsFlow)
{
    int nLevels = pyd1.nlevels();
    float ratio = pyd1.ratio();
    ...
    {
        // neighboring propogation and random searching, update current level's seedsFlow and bestcost
        int iCnt = Propogate(pyd1, pyd2, im1f, im2f, l, searchRadius, iterCnts[l], pydSeeds, neighbors, pydSeedsFlow, bestCosts);
        if (l > 0)
        {
            // compute searching radius for next level
...
void CPM::OnePass(FImagePyramid& pyd1, FImagePyramid& pyd2, UCImage* im1f, UCImage* im2f, 
                  IntImage& seeds,          // seed points in the finest level / original image
                  IntImage& neighbors,      // neighbors of the seeds, the finest level
                  FImage* pydSeedsFlow)
{
    FImage rawImg1 = pyd1[0];
    FImage rawImg2 = pyd2[0];
    ...
    for (int i = 0; i < nLevels; i++)
    {
        pydSeeds[i].allocate(2, numV);   // each level has the same seed count as the finest level
        int sw = pyd1[i].width();
...
double CPM::dist(const Point& a, const Point& b) { return sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y)); }

// intersection between two lines
CPM::Point CPM::intersection(const Point& u1, const Point& u2, const Point& v1, const Point& v2)
{
    Point ans = u1;
...
CPM::Point CPM::circumcenter(const Point& a, const Point& b, const Point& c)
{
    Point ua, ub, va, vb;
    ua.x = (a.x + b.x) / 2;
    ua.y = (a.y + b.y) / 2;
