...
public:
    ...
    template <class T> static bool unknown_flow(T u, T v);
    template <class T> static bool unknown_flow(T* f);

    // read a flow file into 2-band image
    template <class T> static int ReadFlowFile(T* U, T* V, int* w, int* h, const char* filename);

    // write a 2-band image into flow file
    template <class T> static int WriteFlowFile(T* U, T* V, int w, int h, const char* filename);

    // read a KITTI flow file into 2-band image
    template <class T> static int ReadKittiFlowFile(T* U, T* V, int* w, int* h, const char* filename);

    // write a 2-band image into KITTI flow file
    template <class T> static int WriteKittiFlowFile(T* U, T* V, int w, int h, const char* filename);

    // render the motion to a 4-band BGRA color image
    template <class T> static double MotionToColor(unsigned char* fillPix, T* U, T* V, int w, int h, float range = -1);

    template <class T> static float ShowFlow(const char* winname, T* U, T* V, int w, int h, float range = -1, int waittime = 1);
    template <class T> static void SaveFlowAsImage(const char* imgName, T* U, T* V, int w, int h, float range = -1);

    template <class T> static float ErrorImage(unsigned char* fillPix, T* u1, T* v1, T* u2, T* v2, int w, int h);
    template <class T> static float ErrorImage(unsigned char* fillPix, T* u1, T* v1, char* gtName, int w, int h);
    template <class T> static float ShowErrorImage(const char* winname, T* U, T* V, char* gtName, int w, int h, int waittime = 1);
    template <class T> static float SaveErrorImage(const char* imgName, T* U, T* V, char* gtName, int w, int h);

    template <class T1, class T2> static FlowErr CalcFlowError(T1* u1, T1* v1, T2* u2, T2* v2, int w, int h);
...
#define MAXWHEELCOLS 60
    template <class T> static void setcols(T* colorwheel, int r, int g, int b, int k);
    template <class T> static int makecolorwheel(T* colorwheel);
    template <class T> static void computeColor(double fx, double fy, unsigned char* pix, T* colorwheel, int ncols);
};

template <class T> int OpticFlowIO::ReadKittiFlowFile(T* U, T* V, int* w, int* h, const char* filename)
{
    if (filename == NULL)
    {
...
template <class T> int OpticFlowIO::WriteKittiFlowFile(T* U, T* V, int w, int h, const char* filename)
{
    if (filename == NULL)
    {
...
template <class T> bool OpticFlowIO::unknown_flow(T u, T v)
{
    return (abs(u) > UNKNOWN_FLOW_THRESH) || (abs(v) > UNKNOWN_FLOW_THRESH) || u != u || v != v; // isnan()
}

template <class T> bool OpticFlowIO::unknown_flow(T* f) { return unknown_flow(f[0], f[1]); }

template <class T> int OpticFlowIO::ReadFlowFile(T* U, T* V, int* w, int* h, const char* filename)
{
    if (filename == NULL)
    {
    ...
    if ((int)fread(&tag, sizeof(float), 1, stream) != 1 || (int)fread(&width, sizeof(int), 1, stream) != 1
...
template <class T> int OpticFlowIO::WriteFlowFile(T* U, T* V, int w, int h, const char* filename)
{
    if (filename == NULL)
    {
...
template <class T> double OpticFlowIO::MotionToColor(unsigned char* fillPix, T* U, T* V, int w, int h, float range /*= -1*/)
{
    ...
    { // obtain the motion range according to the max flow
...
template <class T>
float OpticFlowIO::ShowFlow(const char* winname, T* U, T* V, int w, int h, float range /*= -1*/, int waittime /*= 1*/)
{
...
template <class T> void OpticFlowIO::SaveFlowAsImage(const char* imgName, T* U, T* V, int w, int h, float range /*= -1*/)
{
    cv::Mat img(h, w, CV_8UC4);
    float maxFlow = OpticFlowIO::MotionToColor(img.data, U, V, w, h, range);
...
template <class T> float OpticFlowIO::ErrorImage(unsigned char* fillPix, T* u1, T* v1, T* u2, T* v2, int w, int h)
{
    unsigned char pix[4];

//#define LOG_COLOR
#ifdef LOG_COLOR
    float LC[10][5] = {{0, 0.0625, 49, 54, 149},      {0.0625, 0.125, 69, 117, 180}, {0.125, 0.25, 116, 173, 209},
                       {0.25, 0.5, 171, 217, 233},    {0.5, 1, 224, 243, 248},       {1, 2, 254, 224, 144},
                       {2, 4, 253, 174, 97},          {4, 8, 244, 109, 67},          {8, 16, 215, 48, 39},
...
template <class T> float OpticFlowIO::ErrorImage(unsigned char* fillPix, T* u1, T* v1, char* gtName, int w, int h)
{
    int gtw, gth;
    T* u2 = new T[w * h];
    T* v2 = new T[w * h];
...
template <class T> float OpticFlowIO::SaveErrorImage(const char* imgName, T* U, T* V, char* gtName, int w, int h)
{
    cv::Mat img(h, w, CV_8UC4);
    float r = OpticFlowIO::ErrorImage(img.data, U, V, gtName, w, h);
...
template <class T1, class T2> FlowErr OpticFlowIO::CalcFlowError(T1* u1, T1* v1, T2* u2, T2* v2, int w, int h)
{
    FlowErr stat;
...
template <class T> void OpticFlowIO::setcols(T* colorwheel, int r, int g, int b, int k)
{
    colorwheel[k * 3 + 0] = r;
    colorwheel[k * 3 + 1] = g;
    colorwheel[k * 3 + 2] = b;
...
template <class T> int OpticFlowIO::makecolorwheel(T* colorwheel)
{
    ...
    for (i = 0; i < GC; i++)
        setcols(colorwheel, 0, 255, 255 * i / GC, k++);
    for (i = 0; i < CB; i++)
...
template <class T> void OpticFlowIO::computeColor(double fx, double fy, unsigned char* pix, T* colorwheel, int ncols)
{
