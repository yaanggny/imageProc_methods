...
static void gck_2d_horiz(int *cur, int a, int *prev, int b, int w, int h, int klen)
{
    ...
    for (j = 0; j < hlen; j++)
    {
        ...
        {
            if (sign)
                cur[i] = prev[i - delta] - cur[i - delta] - prev[i];
            else
                cur[i] = cur[i - delta] - prev[i - delta] - prev[i];
        }
        cur += len;
        prev += len;
    }
...
static void gck_2d_vert(int *cur, int a, int *prev, int b, int w, int h, int klen)
{
    ...
    for (; i < hlen; i++)
    {
        ...
        {
            if (sign)
                cur[j] = prevd[j] - curd[j] - prev[j];
            else
                cur[j] = curd[j] - prevd[j] - prev[j];
        }
        cur += wlen;
        curd += wlen;
        prev += wlen;
...
static int *gck_calc_2d(uint8_t *data, int w, int h, int kern_size, int bases)
{
    ...
    for (i = 1; i < bases; i++)
    {
        ...
        int *cur = res + i * wh;
        if (horiz)
            gck_2d_horiz(cur, path[i].x, prev, adj.x, w, h, kern_size);
        else
            gck_2d_vert(cur, path[i].y, prev, adj.y, w, h, kern_size);
    }
...
static void print_bases(int *data, int w, int h, int kern_size, int bases)
{
