...
static int *gck_calc(int *data, int data_len, int kern_len, int bases)
{
    ...
    int *res = malloc(len * sizeof(int) * bases), *p = res,
        *q = res; // each kernel generates a result vector of length len
    ...
    for (i = 1; i < bases; i++)
    {
        int prefix = gck_prefix(gck_gc(i - 1), gck_gc(i),
                                bits); // prefix length of alpha-index (not prefix itself)
